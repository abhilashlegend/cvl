...
jQuery('.rm_form_el_name > label').attr('for','name');
jQuery('.rm_form_el_name > input').attr('id','name');

jQuery('.rm_form_el_email > label').attr('for','email');
jQuery('.rm_form_el_email > input').attr('id','email');

jQuery('.rm_required_field').attr('id', 'rm_required_field');
jQuery('.rm_required_field').prepend("<label aria-hidden='true' for='rm_required_field'>Required</label>");

function changeNavText(x) {

  if (x.matches) { // If media query matches
    jQuery('#previous-nav').html("&laquo;");
    jQuery('#next-nav').html("&raquo;");
  } else {
   	jQuery('#previous-nav').html("&laquo; Previous Page");
   	jQuery('#next-nav').html("Next Page &raquo;");
  }
}

var x = window.matchMedia("(max-width: 414px)")
changeNavText(x) // Call listener function at run time
x.addListener(changeNavText) // Attach listener function on state changes
...
$('#charitable_field_country_element option[value="US"]').attr("selected",true);
$('#charitable_field_postcode label').text("Zip Code");
$('#charitable-donation-form').attr("autocomplete", "off");
