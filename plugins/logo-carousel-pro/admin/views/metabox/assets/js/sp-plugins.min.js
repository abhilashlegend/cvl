(function () {
    var $, AbstractChosen, Chosen, SelectParser, _ref, __hasProp = {}.hasOwnProperty, __extends = function (child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key] } function ctor() { this.constructor = child } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child }; SelectParser = (function () {
        ...
        SelectParser.prototype.add_node = function (child) { if (child.nodeName.toUpperCase() === "OPTGROUP") { return this.add_group(child) } else { return this.add_option(child) } }; SelectParser.prototype.add_group = function (group) {
            ...
            var map, unsafe_chars; if ((text == null) || text === !1) { return "" }
        ...
        AbstractChosen.prototype.set_default_values = function () { var _this = this; this.click_test_action = function (evt) { return _this.test_active_click(evt) }; this.activate_action = function (evt) { return _this.activate_field(evt) }; this.active_field = !1; this.mouse_on_container = !1; this.results_showing = !1; this.result_highlighted = null; this.allow_single_deselect = (this.options.allow_single_deselect != null) && (this.form_field.options[0] != null) && this.form_field.options[0].text === "" ? this.options.allow_single_deselect : !1; this.disable_search_threshold = this.options.disable_search_threshold || 0; this.disable_search = this.options.disable_search || !1; this.enable_split_word_search = this.options.enable_split_word_search != null ? this.options.enable_split_word_search : !0; this.group_search = this.options.group_search != null ? this.options.group_search : !0; this.search_contains = this.options.search_contains || !1; this.single_backstroke_delete = this.options.single_backstroke_delete != null ? this.options.single_backstroke_delete : !0; this.max_selected_options = this.options.max_selected_options || Infinity; this.inherit_select_classes = this.options.inherit_select_classes || !1; this.display_selected_options = this.options.display_selected_options != null ? this.options.display_selected_options : !0; this.display_disabled_options = this.options.display_disabled_options != null ? this.options.display_disabled_options : !0; return this.include_group_label_in_selected = this.options.include_group_label_in_selected || !1 }; AbstractChosen.prototype.set_default_text = function () {
            ...
            var result, _i, _len, _ref, _results; _ref = this.results_data; _results = []; for (_i = 0, _len = _ref.length; _i < _len; _i++) { result = _ref[_i]; if (result.selected) { _results.push(result.selected = !1) } else { _results.push(void 0) } }
            return _results
            ...
            var escapedSearchText, option, regex, results, results_group, searchText, startpos, text, zregex, _i, _len, _ref; this.no_results_clear(); results = 0; searchText = this.get_search_text(); escapedSearchText = searchText.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"); zregex = new RegExp(escapedSearchText, 'i'); regex = this.get_search_regex(escapedSearchText); _ref = this.results_data; for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                option = _ref[_i]; option.search_match = !1; results_group = null; if (this.include_option_in_results(option)) {
                    if (option.group) { option.group_match = !1; option.active_options = 0 }
                    ...
                    option.search_match = this.search_string_match(option.search_text, regex); if (option.search_match && !option.group) { results += 1 }
                        if (option.search_match) {
                            ...
                            if (results_group != null) { results_group.group_match = !0 }
                        } else if ((option.group_array_index != null) && this.results_data[option.group_array_index].search_match) { option.search_match = !0 }
                    }
                }
            }
            ...
            var stroke, _ref; stroke = (_ref = evt.which) != null ? _ref : evt.keyCode; this.search_field_scale(); switch (stroke) {
                case 8: if (this.is_multiple && this.backstroke_length < 1 && this.choices_count() > 0) { return this.keydown_backstroke() } else if (!this.pending_backstroke) { this.result_clear_highlight(); return this.results_search() }
                    ...
                    return !0; case 9: case 38: case 40: case 16: case 91: case 17: break; default: return this.results_search()
            }
            ...
            if (this.is_multiple && (!this.display_selected_options && option.selected)) { return !1 }
            if (!this.display_disabled_options && option.disabled) { return !1 }
            if (option.empty) { return !1 }
            return !0
        }; AbstractChosen.prototype.search_results_touchstart = function (evt) { this.touch_started = !0; return this.search_results_mouseover(evt) }; AbstractChosen.prototype.search_results_touchmove = function (evt) { this.touch_started = !1; return this.search_results_mouseout(evt) }; AbstractChosen.prototype.search_results_touchend = function (evt) { if (this.touch_started) { return this.search_results_mouseup(evt) } }; AbstractChosen.prototype.outerHTML = function (element) {
        ...
        this.is_disabled = this.form_field_jq[0].disabled; if (this.is_disabled) {
            this.container.addClass('chosen-disabled'); this.search_field[0].disabled = !0; if (!this.is_multiple) { this.selected_item.unbind("focus.chosen", this.activate_action) }
            return this.close_field()
        } else { this.container.removeClass('chosen-disabled'); this.search_field[0].disabled = !1; if (!this.is_multiple) { return this.selected_item.bind("focus.chosen", this.activate_action) } }
        ...
        this.parsing = !0; this.selected_option_count = null; this.results_data = SelectParser.select_to_array(this.form_field); if (this.is_multiple) { this.search_choices.find("li.search-choice").remove() } else if (!this.is_multiple) { this.single_set_selected_text(); if (this.disable_search || this.form_field.options.length <= this.disable_search_threshold) { this.search_field[0].readOnly = !0; this.container.addClass("chosen-container-single-nosearch") } else { this.search_field[0].readOnly = !1; this.container.removeClass("chosen-container-single-nosearch") } }
            this.update_results_content(this.results_option_build({ first: !0 })); this.search_field_disabled(); this.show_search_field_default(); this.search_field_scale(); return this.parsing = !1
            ...
            if (this.is_multiple && this.max_selected_options <= this.choices_count()) { this.form_field_jq.trigger("chosen:maxselected", { chosen: this }); return !1 }
            this.container.addClass("chosen-with-drop"); this.results_showing = !0; this.search_field.focus(); this.search_field.val(this.search_field.val()); this.winnow_results(); return this.form_field_jq.trigger("chosen:showing_dropdown", { chosen: this })
            ...
            var high, item; if (this.result_highlight) {
                high = this.result_highlight; this.result_clear_highlight(); if (this.is_multiple && this.max_selected_options <= this.choices_count()) { this.form_field_jq.trigger("chosen:maxselected", { chosen: this }); return !1 }
            ...
            result_data.selected = !1; this.form_field.options[result_data.options_index].selected = !1; this.selected_option_count = null; this.result_clear_highlight(); if (this.results_showing) { this.winnow_results() }
                this.form_field_jq.trigger("change", { deselected: this.form_field.options[result_data.options_index].value }); this.search_field_scale(); return !0
            } else { return !1 }
    ...
    "use strict"; function log(msg) { if (window.console && window.console.log) { console.log(msg) } }
    function safeFind(context, selector) {
        if (selector[0] == "#") { if (selector.indexOf(" ") < 0) { return $(selector) } }
        return context.find(selector)
    }
    var configExample = { show: null, hide: null, log: !1, checkTargets: !0 }; function Rule(controller, condition, value) { this.init(controller, condition, value) }
    $.extend(Rule.prototype, {
        init: function (controller, condition, value) { this.controller = controller; this.condition = condition; this.value = value; this.rules = []; this.controls = [] }, evalCondition: function (context, control, condition, val1, val2) { if (condition == "==") { return this.checkBoolean(val1) == this.checkBoolean(val2) } else if (condition == "!=") { return this.checkBoolean(val1) != this.checkBoolean(val2) } else if (condition == ">=") { return Number(val2) >= Number(val1) } else if (condition == "<=") { return Number(val2) <= Number(val1) } else if (condition == ">") { return Number(val2) > Number(val1) } else if (condition == "<") { return Number(val2) < Number(val1) } else if (condition == "()") { return window[val1](context, control, val2) } else if (condition == "any") { return $.inArray(val2, val1.split(',')) > -1 } else if (condition == "not-any") { return $.inArray(val2, val1.split(',')) == -1 } else { throw new Error("Unknown condition:" + condition) } }, checkBoolean: function (value) {
            switch (value) { case true: case 'true': case 1: case '1': value = !0; break; case false: case 'false': case 0: case '0': value = !1; break }
            return value
        }, checkCondition: function (context, cfg) {
            if (!this.condition) { return !0 }
            var control = context.find(this.controller); if (control.size() === 0 && cfg.log) { log("Evaling condition: Could not find controller input " + this.controller) }
            var val = this.getControlValue(context, control); if (cfg.log && val === undefined) { log("Evaling condition: Could not exctract value from input " + this.controller) }
            if (val === undefined) { return !1 }
            val = this.normalizeValue(control, this.value, val); return this.evalCondition(context, control, this.condition, this.value, val)
        }, normalizeValue: function (control, baseValue, val) {
            if (typeof baseValue == "number") { return parseFloat(val) }
            return val
        }, getControlValue: function (context, control) {
            if ((control.attr("type") == "radio" || control.attr("type") == "checkbox") && control.size() > 1) { return control.filter(":checked").val() }
            if (control.attr("type") == "checkbox" || control.attr("type") == "radio") { return control.is(":checked") }
            return control.val()
        }, createRule: function (controller, condition, value) { var rule = new Rule(controller, condition, value); this.rules.push(rule); return rule }, include: function (input) {
            if (!input) { throw new Error("Must give an input selector") }
            this.controls.push(input)
        }, applyRule: function (context, cfg, enforced) {
            var result; if (enforced === undefined) { result = this.checkCondition(context, cfg) } else { result = enforced }
            if (cfg.log) { log("Applying rule on " + this.controller + "==" + this.value + " enforced:" + enforced + " result:" + result) }
            if (cfg.log && !this.controls.length) { log("Zero length controls slipped through") }
            var show = cfg.show || function (control) { control.show() }; var hide = cfg.hide || function (control) { control.hide() }; var controls = $.map(this.controls, function (elem, idx) {
                var control = context.find(elem); if (cfg.log && control.size() === 0) { log("Could not find element:" + elem) }
                return control
            }); if (result) {
                $(controls).each(function () {
                    if (cfg.log && $(this).size() === 0) { log("Control selection is empty when showing"); log(this) }
                    show(this)
                }); $(this.rules).each(function () { this.applyRule(context, cfg) })
            } else {
                $(controls).each(function () {
                    if (cfg.log && $(this).size() === 0) { log("Control selection is empty when hiding:"); log(this) }
                    hide(this)
                }); $(this.rules).each(function () { this.applyRule(context, cfg, !1) })
            }
        }
    }); function Ruleset() { this.rules = [] }
    $.extend(Ruleset.prototype, {
        createRule: function (controller, condition, value) { var rule = new Rule(controller, condition, value); this.rules.push(rule); return rule }, applyRules: function (context, cfg) {
            var i; cfg = cfg || {}; if (cfg.log) { log("Starting evaluation ruleset of " + this.rules.length + " rules") }
            for (i = 0; i < this.rules.length; i++) { this.rules[i].applyRule(context, cfg) }
        }, walk: function () {
            var rules = []; function descent(rule) { rules.push(rule); $(rule.children).each(function () { descent(this) }) }
            $(this.rules).each(function () { descent(this) }); return rules
        }, checkTargets: function (context, cfg) {
            var controls = 0; var rules = this.walk(); $(rules).each(function () {
                if (context.find(this.controller).size() === 0) { throw new Error("Rule's controller does not exist:" + this.controller) }
                if (this.controls.length === 0) { throw new Error("Rule has no controls:" + this) }
                $(this.controls).each(function () {
                    if (safeFind(context, this) === 0) { throw new Error("Rule's target control " + this + " does not exist in context " + context.get(0)) }
                    controls++
                })
            }); if (cfg.log) { log("Controller check ok, rules count " + rules.length + " controls count " + controls) }
        }, install: function (cfg) { $.deps.enable($(document.body), this, cfg) }
    }); var deps = {
        createRuleset: function () { return new Ruleset() }, enable: function (selection, ruleset, cfg) {
            cfg = cfg || {}; if (cfg.checkTargets || cfg.checkTargets === undefined) { ruleset.checkTargets(selection, cfg) }
            var self = this; if (cfg.log) { log("Enabling dependency change monitoring on " + selection.get(0)) }
            var handler = function () { ruleset.applyRules(selection, cfg) }; var val = selection.on ? selection.on("change.deps", null, null, handler) : selection.live("change.deps", handler); ruleset.applyRules(selection, cfg); return val
        }
    }; $.deps = deps
})(jQuery); +function ($) {
    ...
    'use strict'; var SPTooltip = function (element, options) {
    this.type = null
        this.options = null
        this.enabled = null
        this.timeout = null
    ...
    SPTooltip.DEFAULTS = { animation: !0, placement: 'top', selector: !1, template: '<div class="sp-tooltip" role="tooltip"><div class="sp-tooltip-arrow"></div><div class="sp-tooltip-inner"></div></div>', trigger: 'hover focus', title: '', delay: 0, html: !1, container: !1, viewport: { selector: 'body', padding: 0 } }
    ...
    SPTooltip.prototype.getDefaults = function () { return SPTooltip.DEFAULTS }
    ...
    SPTooltip.prototype.show = function () {
        ...
        if (this.hasContent() && this.enabled) {
            ...
            $.support.transition && this.$tip.hasClass('fade') ? $tip.one('bsTransitionEnd', complete).CSemulateTransitionEnd(SPTooltip.TRANSITION_DURATION) : complete()
        }
    }
    ...
    SPTooltip.prototype.replaceArrow = function (delta, dimension, isVertical) { this.arrow().css(isVertical ? 'left' : 'top', 50 * (1 - delta / dimension) + '%').css(isVertical ? 'top' : 'left', '') }
    ...
    SPTooltip.prototype.hide = function (callback) {
        ...
        $tip.removeClass('in')
        $.support.transition && $tip.hasClass('fade') ? $tip.one('bsTransitionEnd', complete).CSemulateTransitionEnd(SPTooltip.TRANSITION_DURATION) : complete()
        this.hoverState = null
        return this
    }
    ...
    SPTooltip.prototype.hasContent = function () { return this.getTitle() }
    ...
    SPTooltip.prototype.getCalculatedOffset = function (placement, pos, actualWidth, actualHeight) { return placement == 'bottom' ? { top: pos.top + pos.height, left: pos.left + pos.width / 2 - actualWidth / 2 } : placement == 'top' ? { top: pos.top - actualHeight, left: pos.left + pos.width / 2 - actualWidth / 2 } : placement == 'left' ? { top: pos.top + pos.height / 2 - actualHeight / 2, left: pos.left - actualWidth } : { top: pos.top + pos.height / 2 - actualHeight / 2, left: pos.left + pos.width } }
    ...
    SPTooltip.prototype.tip = function () { return (this.$tip = this.$tip || $(this.options.template)) }
    SPTooltip.prototype.arrow = function () { return (this.$arrow = this.$arrow || this.tip().find('.sp-tooltip-arrow')) }
    SPTooltip.prototype.enable = function () { this.enabled = !0 }
    SPTooltip.prototype.disable = function () { this.enabled = !1 }
    SPTooltip.prototype.toggleEnabled = function () { this.enabled = !this.enabled }
