 ...
 *
 */
abstract class SP_LCPRO_Framework_Options extends SP_LCPRO_Framework_Abstract {
	...
	public function element_after() {
		...
		$out .= ( isset( $this->field['after'] ) ) ? '<span class="sp-text-after">'. $this->field['after'].'</span>' : '';
	...
	public function element_debug() {
		...
		if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'SP_LCPRO_OPTIONS_DEBUG' ) && SP_LCPRO_OPTIONS_DEBUG ) ) {

			$value = $this->element_value();

			$out .= "<pre>";
			$out .= "<strong>" . __( 'CONFIG', 'logo-carousel-pro' ) . ":</strong>";
			$out .= "\n";
			ob_start();
			var_export( $this->field );
			$out .= htmlspecialchars( ob_get_clean() );
			$out .= "\n\n";
			$out .= "<strong>" . __( 'USAGE', 'logo-carousel-pro' ) . ":</strong>";
			$out .= "\n";
			$out .= ( isset( $this->field['id'] ) ) ? "sp_lcpro_get_option( '". $this->field['id'] ."' );" : '';

			if( ! empty( $value ) ) {
				$out .= "\n\n";
				$out .= "<strong>". __( 'VALUE', 'logo-carousel-pro' ) .":</strong>";
				$out .= "\n";
				ob_start();
				var_export( $value );
				$out .= htmlspecialchars( ob_get_clean() );
		...
		if( ( isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true ) || ( defined( 'SP_LCPRO_OPTIONS_DEBUG_LIGHT' ) && SP_LCPRO_OPTIONS_DEBUG_LIGHT ) ) {
			...
			$out .= ( isset( $this->field['id'] ) ) ? "sp_lcpro_get_option( '". $this->field['id'] ."' );" : '';
			$out .= "\n";
			$out .= "<strong>". __( 'ID', 'logo-carousel-pro' ) .":</strong>";
			$out .= "\n";
		...
		if ( is_array( $this->multilang ) ) {
			...
			foreach ( $this->multilang['languages'] as $key => $val ) {
				...
				if( $key != $this->multilang['current'] ) {
					...
					$class = 'SP_LCPRO_Framework_Option_' . $this->field['type'];
			...
			$out .= '</fieldset>';
			$out .= '<p class="sp-text-desc">'. sprintf( __( 'You are editing language: ( <strong>%s</strong> )', 'logo-carousel-pro' ), $this->multilang['current'] ) .'</p>';
