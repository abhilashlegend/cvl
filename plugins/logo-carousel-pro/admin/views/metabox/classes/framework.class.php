 ...
 *
 */
class SP_LCPRO_Framework extends SP_LCPRO_Framework_Abstract {
	...
	public function __construct( $settings, $options ) {
		...
		if( ! empty( $this->options ) ) {
			...
			$this->get_option = get_option( SP_LCPRO_OPTION );
	...
	public static function instance( $settings = array(), $options = array() ) {
		if ( is_null( self::$instance ) && SP_ACTIVE_FRAMEWORK ) {
	...
	public function validate_save( $request ) {
		...
		if ( isset( $request['resetall'] ) ) {
			$add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'logo-carousel-pro' ), 'updated' );
			return;
		}
		...
		if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
			...
			$add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'logo-carousel-pro' ), 'updated' );
		}
		...
		set_transient( 'sp_lcpro_framework_transient', array( 'errors' => $add_errors, 'section_id' => $section_id ), $transient_time );
	...
	public function field_callback( $field ) {
		...
		echo sp_add_element( $field, $value, $this->unique );
	}
	...
	public function admin_page() {
		...
		$transient  = get_transient( 'sp_lcpro_framework_transient' );
		...
		echo '<div class="sp-framework sp-option-framework">';
		...
		echo ( $this->settings['ajax_save'] ) ? '<span id="sp-save-ajax">'. esc_html__( 'Settings saved.', 'logo-carousel-pro' ) .'</span>' : '';

		submit_button( esc_html__( 'Save Changes', 'logo-carousel-pro' ), 'primary sp-save', 'save', false, array( 'data-save' => esc_html__( 'Saving...', 'logo-carousel-pro' ) ) );
		submit_button( esc_html__( 'Restore', 'logo-carousel-pro' ), 'secondary sp-restore sp-reset-confirm', $this->unique .'[reset]', false );

		if( $this->settings['show_reset_all'] ) {
			submit_button( esc_html__( 'Reset All Options', 'logo-carousel-pro' ), 'secondary sp-restore sp-warning-primary sp-reset-confirm', $this->unique . '[resetall]' , false );
		}

		echo '</fieldset>';
		//echo ( empty( $has_nav ) ) ? '<a href="#" class="sp-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'logo-carousel-pro' ) .'</a>' : '';
